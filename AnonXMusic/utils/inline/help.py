...
def help_pannel(_, page: int = 0, START: Union[bool, int] = None):
    ...
    navigation_buttons = []
    if page > 0:
        navigation_buttons.append(InlineKeyboardButton(text="⬅ Back", callback_data=f"help_page {page - 1}"))
    if page < len(pages) - 1:
        navigation_buttons.append(InlineKeyboardButton(text="Next ➡", callback_data=f"help_page {page + 1}"))

    # Format buttons in rows of 3
    button_rows = [pages[page][i:i + 3] for i in range(0, 15, 3)]
    if navigation_buttons:
        button_rows.append(navigation_buttons)

    # Add final row with close/back button
